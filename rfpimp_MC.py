...
def importances(model, X_valid, y_valid, features=None, n_samples=5000, sort=True, metric=None, sample_weights=None):
    ...
    X_valid, y_valid, sample_weights = sample(
        X_valid, y_valid, n_samples, sample_weights=sample_weights)
...
def sample(X_valid, y_valid, n_samples, sample_weights=None):
    ...
    if n_samples < len(X_valid):
        ...
        if sample_weights is not None:
            sample_weights = sample_weights.iloc[ix].copy(deep=False)
...
def cv_importances(model, X_train, y_train, k=3):
    ...
    I = pd.DataFrame(data={'Feature': X_train.columns,
                     'Importance': np.array(imp)})
    I = I.set_index('Feature')
    I = I.sort_values('Importance', ascending=False)
    return I
...
def dropcol_importances(model, X_train, y_train, X_valid=None, y_valid=None, metric=None, sample_weights=None):
    ...
    for col in X_train.columns:
        ...
        if callable(metric):
            s = metric(model_, X_valid.drop(
                col, axis=1), y_valid, sample_weights)
        else:
            s = model_.score(X_valid.drop(col, axis=1),
                             y_valid, sample_weights)
...
def stemplot_importances(df_importances,
    ...
    maxdrop = np.max(imp)
    imp_padding = 0.002
    imp_range = (min(imp_range[0], mindrop -
                 imp_padding), max(imp_range[1], maxdrop))
    ...
    if title:
        ax.set_title(title, fontsize=label_fontsize +
                     1, fontname="Arial", color=GREY)

    plt.hlines(y=yloc, xmin=imp_range[0],
               xmax=imp, lw=barcounts*1.2, color=color)
...
def plot_importances(df_importances,
    ...
    maxdrop = np.max(imp)
    imp_padding = 0.002
    imp_range = (min(imp_range[0], mindrop - imp_padding),
                 max(imp_range[1], maxdrop + imp_padding))
...
def feature_dependence_matrix(X_train,
    ...
    cat_cols = [
        col for col in numeric_cols if X_train[col].value_counts().count() <= cat_count]
    ...
    df_dep = pd.DataFrame(index=X_train.columns, columns=[
                          'Dependence']+X_train.columns.tolist())
    for i, col in enumerate(numeric_cols):
        X, y = X_train.drop(col, axis=1), X_train[col]
        if col in cat_cols:
            rf = clone(rfcmodel)
            rf.fit(X, y)
            imp = permutation_importances_raw(
                rf, X, y, oob_classifier_f1_score, n_samples)
        else:
            rf = clone(rfrmodel)
            rf.fit(X, y)
            imp = permutation_importances_raw(
                rf, X, y, oob_regression_r2_score, n_samples)
        """
        ...
        # add overall dependence
        df_dep.iloc[i] = np.insert(imp, 0, rf.oob_score_)
...
def plot_dependence_heatmap(D,
    ...
    if cmap is None:
        ...
        cmap = ListedColormap([cw(x)
                              for x in np.arange(color_threshold, .85, 0.01)])
    ...
    im = plt.imshow(depdata, cmap=cm, vmin=color_threshold,
...
def plot_corr_heatmap(df,
    ...
    im = plt.imshow(filtered, cmap=cm, vmin=color_threshold,
                    vmax=1, aspect='equal')
    ...
    cb = plt.colorbar(im, fraction=0.046, pad=0.04, ticks=[
                      color_threshold, color_threshold + (1 - color_threshold) / 2, 1.0])
